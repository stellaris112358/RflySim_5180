...
class Config:
    ...
    MAP_CONFIG = {
        'resolution': 0.1,           # 地图分辨率，单位：米/像素，每个像素代表0.05米
        'inflation_radius': 1,      # 障碍物膨胀半径，单位：米，为路径规划提供安全边界
...
class PathPlanningApp:
    ...
    def __init__(self):
        ...
        self.start_pixel = [0, 1, -2]  # [行, 列, 高度] 起始点的像素坐标和高度信息
        self.goal_pixel = [-10, -20, -4]   # [行, 列, 高度] 目标点的像素坐标和高度信息
    ...
    def run(self, start=None, goal=None):
        """运行应用程序"""
        if start and goal:
            self.start_pixel = self.world2pixel(start)  # [行, 列, 高度] 起始点的像素坐标和高度信息
            self.goal_pixel = self.world2pixel(goal)   # [行, 列, 高度] 目标点的像素坐标和高度信息
            print("start", start)
            print("start_pixel", self.start_pixel)
        else:
            self.start_pixel = self.world2pixel(self.start_pixel)  # [行, 列, 高度] 起始点的像素坐标和高度信息
            self.goal_pixel = self.world2pixel(self.goal_pixel)   # [行, 列, 高度] 目标点的像素坐标和高度信息
        ...
        if not self._validate_coordinates():
            self.start_pixel = self.snap2free(self.start_pixel)
            self.goal_pixel = self.snap2free(self.goal_pixel)
            if not (self.start_pixel and self.goal_pixel):
                return None
            print("起点或终点移动成功!")
        # 执行路径规划
        ...
        if start and goal:
            return self.pixel2world(path_pixels)
    ...
    def _load_and_process_map(self):
        """加载和处理地图"""
        img_path = sys.path[0] + "\\pursuer_and_evader_s.pgm"
        self.grid, self.img, self.gaussian_grid = MapProcessor.preprocess_map(img_path)
        return self.grid is not None
    
    def _load_map_yaml(self):
        SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
        yaml_path = os.path.join(SCRIPT_DIR, 'pursuer_and_evader_s.yaml')
        with open(yaml_path, 'r', encoding='utf-8') as f:
            info = yaml.safe_load(f) 
        self.origin_x, self.origin_y, self.origin_yaw = info['origin']
        self.resolution = info['resolution']
        Config.MAP_CONFIG['resolution'] = self.resolution
        print("resolution:", Config.MAP_CONFIG['resolution'])
    
    def pixel2world(self, path_pixels):
        if not path_pixels:
            return path_pixels
        world_path = []
        for pixel_point in path_pixels:
            world_x = pixel_point[1] * self.resolution + self.origin_x  # 列坐标 -> x
            world_y = pixel_point[0] * self.resolution + self.origin_y  # 行坐标 -> y  
            world_z = -pixel_point[2] * self.resolution # 高度坐标 -> z
            world_path.append([world_x, world_y, world_z])
        return world_path

    def world2pixel(self, point):
        pixel_x = (point[0] - self.origin_x) / self.resolution  # 列坐标 -> x
        pixel_y = (point[1] - self.origin_y) / self.resolution  # 行坐标 -> y  
        pixel_z = -(point[2]) / self.resolution  # 高度坐标 -> z
    ...
    def _check_coordinate_bounds(self):
        ...
        if (self.start_pixel[0] < 0 or self.start_pixel[0] >= grid_rows or 
            ...
            self.start_pixel[2] < 0 or self.start_pixel[2] >= 150 or 
            self.goal_pixel[2] < 0 or self.goal_pixel[2] >= 150):
            
            print("起点或终点坐标超出地图范围!")
            print(f"地图范围: 行[0, {grid_rows-1}], 列[0, {grid_cols-1}], 高度[0, 149]")
            return False
    ...
    def snap2free(self, pixel, max_radius=50):
        h, w = self.img.shape[0], self.img.shape[0]
        y, x = pixel[0], pixel[1]
        if self.gaussian_grid[y][x] == 0:          # 已经在自由区
            return pixel
        q = deque([(y, x, 0)])
        visited = np.zeros_like(self.grid, dtype=bool)
        visited[y, x] = True
        dirs = [(-1,0),(1,0),(0,-1),(0,1)]          # 4-连通
        dirs += [(-1,-1),(-1,1),(1,-1),(1,1)]     # 8-连通可打开

        while q:
            cy, cx, d = q.popleft()
            if d >= max_radius:          # 搜索范围到顶
                break
            for dy, dx in dirs:
                ny, nx = cy+dy, cx+dx
                if 0 <= ny < h and 0 <= nx < w and not visited[ny, nx]:
                    if self.gaussian_grid[ny][nx] == 0:          # 找到自由栅格
                        return [ny, nx, pixel[-1]]
                    visited[ny, nx] = True
                    q.append((ny, nx, d+1))
        print("移动起点或终点失败")
        return None          # 失败
