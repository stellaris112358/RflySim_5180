    ...
    if path:
        ...
        for i in range(len(path)-5):
            ...
            if dis < distance:
                ...
                yaw = calc_yaw_from_two_points(pos[:3], path[i+4])
                Xr = np.array([path[i+4][0], path[i+4][1], path[i+4][2], yaw])
                idx = i + 1
                break
        # if len(path) - idx > 6:
        #     for i in range(5):
...
def astar_process_func(position_queue, goal_queue, path_queue, num_drones):
    ...
    while True:
        try:
            ...
            goal = [world2pixel(pos) for pos in drone_goal]
            # goal = [
...
if __name__ == "__main__":
    ...
    #     if i == 0:
    #         ukf1 = UKF(pos, timeInterval)
    #     elif i == 1:
    #         ukf2 = UKF(pos, timeInterval)   
    #     elif i == 2:
    #         ukf3 = UKF(pos, timeInterval)   
    ...
    while True:
            ...
            if current_path:
                if current_path[i]:
                    ...
                    if idx:
                        ...
                        # print(f"{i+1}",x)
                        MavList[i+1].SendPosNED(x[0], x[1], x[2], Xr[3])
        ...
        if current_path and current_flag == 0:
            # 绘制无人机1的路径（ID 6~105）
            for i in range(min(len(current_path[0]), 50)):
