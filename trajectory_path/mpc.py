...
class uav_mpc:
    def __init__(self):
        self.T = 1 / 30
        self.umin=np.array([-0.1, -0.8, -0.1])
        self.umax=np.array([0.2, 0.8, 0.1])
        self.q = np.array([1, 1, 1, 1])
        self.r = np.array([0.1, 0.1, 0.1])
    ...
    def step(self, Ur, Xr, X, Np):
        ...
        A=np.array([[1, 0, 0, -Ur[0, 0]*self.T*np.sin(Xr[3])],
            [0, 1, 0, Ur[0, 0]*self.T*np.cos(Xr[3])],
            [0, 0, 1, 0],
            [0, 0, 0, 1]])
        B=np.array([[self.T*np.cos(Xr[3]), 0, 0],
                    [self.T*np.sin(Xr[3]), 0, 0],
                    [0, 0, self.T],
                    [0, self.T, 0]])
